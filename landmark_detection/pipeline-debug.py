...
class DummyPostprocessModule(nn.Module):
    """Módulo de postprocesado que ignora sus entradas y devuelve valores constantes."""

    def __init__(self):
        super().__init__()
        self.register_buffer("boxes_const", torch.zeros((1, 4), dtype=torch.float32))
        self.register_buffer("scores_const", torch.zeros((1,), dtype=torch.float32))
        self.register_buffer("classes_const", torch.zeros((1,), dtype=torch.int64))

    def forward(self, final_boxes, final_scores, final_classes, orig_size):
        return self.boxes_const, self.scores_const, self.classes_const


class Pipeline_Landmark_Detection_Debug():
    ...
    def __init__(
        ...
        pipeline_onnx_file: str = "pipeline-debug.onnx",
        ...
        self.postprocess_onnx_path = os.path.join(module_dir, "models", "postprocess-debug.onnx")
        self.postprocess_module = DummyPostprocessModule()
        self._export_postprocess(self.postprocess_module, self.postprocess_onnx_path)
    ...
    def _export_postprocess(self, postprocessor, postprocess_onnx_path: str):
        ...
        dummy_orig = torch.zeros((2,), dtype=torch.float32)
